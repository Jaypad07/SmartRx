...
//            userObject.setPassword(userObject.getPassword());
    ...
    public ResponseEntity<?> loginUser(@RequestBody LoginRequest loginRequest) {
        try {
            ResponseEntity<?> loggedInUser = userRepository.loginUser(loginRequest);
            return ResponseEntity.ok(loggedInUser);
        } catch (UnauthorizedException e) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build(); //constructs the response entity object
        } catch (Exception e) {
            e.printStackTrace();
